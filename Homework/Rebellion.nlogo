...
breed [ cops cop ]
breed [ provocateurs provocateur ]

globals [
  k
  threshold
  initial-provocateurs-number
]
...
to setup
  ...
  set k 2.3
  set threshold 0.1
  set initial-provocateurs-number 10  ; Наприклад, початкова кількість провокаторів - 5
  ask patches [
    set pcolor gray - 1
  ...
  if initial-cop-density + initial-agent-density > 100 [
    user-message "The sum of INITIAL-COP-DENSITY and INITIAL-AGENT-DENSITY should not be greater than 100."
    stop
  ]
  ...
  ; Створення провокаторів
  create-provocateurs initial-provocateurs-number [
    move-to one-of patches with [ not any? turtles-here ]
    set influence-radius 3  ; Встановлення радіусу впливу
    display-provocateur
  ]
...
to go
  ask turtles [
    if (breed = agents and jail-term = 0) or breed = cops or breed = provocateurs [ move ]

    if breed = agents and jail-term = 0 [ determine-behavior ]

    if breed = cops [ enforce ]

    if breed = provocateurs [ provoke ]
  ]
...
to provoke
  ...
  let targets agents in-radius influence-radius
  ask targets [
    if not active? and not any? cops in-radius 1 [
      set active? true
    ]
  ]
end
...
to move  ; turtle procedure
  if (breed = agents and jail-term = 0) or (breed = cops) or (breed = provocateurs and jail-term = 0) [
...
to-report grievance
  let economic-factor (1 - living-standard) + unemployment-rate
  report (perceived-hardship + economic-factor) / 2 * (1 - government-legitimacy)
end
...
to enforce
  if any? (agents-on neighborhood) with [active?] or any? (provocateurs-on neighborhood) [
    let target one-of (turtles-on neighborhood) with [breed = agents and active? or breed = provocateurs]
    move-to target
    ask target [
      set active? false
...
to display-provocateur
  set color yellow
   if jail-term > 0
        [ set color black + 3 ]
  ifelse visualization = "2D"
    [ set shape "square" ]  ;
    [ set shape "person provocateur" ]  ;
end
